...
import type { ProductForm } from '../../new/useNewProduct';
import { CURRENCIES } from '../../new/useNewProduct';
...
export function useEditProduct(id: string) {
    ...
    api.get(`/products/${id}`)
      .then(({ data }) => {
        ...
        setMedia(sorted.map((img: any) => ({
          id: img.id, url: img.url, publicId: img.publicId,
          isPrimary: img.isPrimary, resourceType: img.resourceType || 'image',
        })));
      })
  ...
  const discountPct      = form.comparePrice && Number(form.comparePrice) > Number(form.price)
    ? Math.round((1 - Number(form.price) / Number(form.comparePrice)) * 100) : 0;
